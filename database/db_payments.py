...
__all__ = [
    ...
    'save_yoomoney_label',
    'find_order_by_yoomoney_label',
    'save_yoomoney_payment_id',
    'find_latest_pending_yoomoney_order_for_user',
...
def save_yoomoney_label(order_id: str, yoomoney_label: str) -> bool:
    """
    Сохраняет YooMoney label в запись ордера.

    Args:
        order_id: Наш внутренний order_id
        yoomoney_label: label платежа в YooMoney (= order_id)
    ...
    with get_db() as conn:
        cursor = conn.execute(
            "UPDATE payments SET yoomoney_label = ? WHERE order_id = ?",
            (yoomoney_label, order_id)
        )
        success = cursor.rowcount > 0
        if success:
            logger.info(f"Сохранён yoomoney_label={yoomoney_label} для order_id={order_id}")
        return success


def save_yoomoney_payment_id(order_id: str, yoomoney_payment_id: str) -> bool:
    """
    Сохраняет ID платежа YooMoney в запись ордера.

    Args:
        order_id: Наш внутренний order_id
        yoomoney_payment_id: ID платежа в системе YooMoney
    ...
    with get_db() as conn:
        cursor = conn.execute(
            ...
            (yoomoney_payment_id, order_id)
        )
        success = cursor.rowcount > 0
        if success:
            logger.info(f"Сохранён yoomoney_payment_id={yoomoney_payment_id} для order_id={order_id}")
        return success

def find_order_by_yoomoney_label(label: str) -> Optional[Dict[str, Any]]:
    """
    Находит ордер по YooMoney label.

    Args:
        label: label платежа в YooMoney
    ...
    with get_db() as conn:
        cursor = conn.execute(
            "SELECT * FROM payments WHERE yoomoney_label = ?",
            (label,)
        )
        row = cursor.fetchone()
        return dict(row) if row else None

def find_latest_pending_yoomoney_order_for_user(user_id: int) -> Optional[Dict[str, Any]]:
    """
    Находит последний pending-ордер типа 'yoomoney' для пользователя.
...
def get_daily_payments_stats() -> Dict[str, Any]:
    ...
    with get_db() as conn:
        ...
        cursor = conn.execute("""
            ...
            LEFT JOIN tariffs t ON p.tariff_id = t.id
            WHERE p.status = 'paid'
            AND p.payment_type = 'yoomoney'
            AND p.paid_at >= datetime('now', '-1 day')
        """)
        yoomoney_row = cursor.fetchone()
        ...
        paid_count = (crypto_row['count'] if crypto_row else 0) + \
                     ...
                     (yoomoney_row['count'] if yoomoney_row else 0)
        ...
        total_rub = (cards_row['total_rub'] if cards_row else 0) + \
                    ...
                    (yoomoney_row['total_rub'] if yoomoney_row else 0)
