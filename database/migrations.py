...
def _renew_payment_page_buttons() -> str:
    ...
    return json.dumps([
        ...
        {"id": "btn_renew_pay_yoomoney", "label": "💳 ЮMoney",                    "color": "secondary",   "row": 7, "col": 0, "is_hidden": False, "action_type": "system", "action_value": None},
...
def migration_37(conn):
    """
    Миграция v37: добавление платёжного метода YooMoney (ЮMoney).

    - Добавляет настройки yoomoney_enabled, yoomoney_client_id, yoomoney_secret_key, yoomoney_access_token
    - Добавляет колонку yoomoney_label в таблицу payments
    - Добавляет кнопку btn_pay_yoomoney в дефолтную раскладку страницы prepayment
    """
    # 1. Настройки YooMoney
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('yoomoney_enabled', '0')")
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('yoomoney_client_id', '')")
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('yoomoney_secret_key', '')")
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('yoomoney_api_token', '')")

    # 2. Колонки для отслеживания платежей YooMoney
    _add_column(conn, "payments", "yoomoney_label TEXT")
    _add_column(conn, "payments", "yoomoney_payment_id TEXT")

    # 3. Обновляем buttons_default страницы prepayment — вставляем btn_pay_yoomoney после btn_pay_cardlink
    cursor = conn.execute("SELECT buttons_default FROM pages WHERE page_key = 'prepayment'")
    row = cursor.fetchone()
    if row:
        try:
        ...
        if 'btn_pay_yoomoney' not in existing_ids:
            cardlink_row = None
            for b in buttons:
                if isinstance(b, dict) and b.get('id') == 'btn_pay_cardlink':
                    cardlink_row = b.get('row', 0)
                    break

            if cardlink_row is None:
                max_row = max((b.get('row', 0) for b in buttons if isinstance(b, dict)), default=-1)
                new_row = max_row + 1
                for b in buttons:
                    if isinstance(b, dict) and b.get('id') == 'btn_back_main':
                ...
                for b in buttons:
                    if isinstance(b, dict) and b.get('row', 0) > cardlink_row:
                        b['row'] = b['row'] + 1
                buttons.append({
                    "id": "btn_pay_yoomoney",
                    "label": "💳 ЮMoney",
            ...
            logger.info("Кнопка btn_pay_yoomoney добавлена в дефолтную раскладку prepayment")

    # 4. Обновляем buttons_default страницы renew_payment — вставляем btn_renew_pay_yoomoney
    ...
    if row:
        ...
        if 'btn_renew_pay_yoomoney' not in existing_ids:
            cardlink_row = None
            for b in buttons:
                if isinstance(b, dict) and b.get('id') == 'btn_renew_pay_cardlink':
                    cardlink_row = b.get('row', 0)
                    break
            ...
            if cardlink_row is None:
                ...
                new_row = max_row + 1
                for b in buttons:
                    if isinstance(b, dict) and b.get('id') == 'btn_renew_back':
                        new_row = b.get('row', new_row)
                        b['row'] = new_row + 1
                        break
                buttons.append({
                    "id": "btn_renew_pay_yoomoney",
                    "label": "💳 ЮMoney",
                    "color": "secondary",
                    "row": new_row,
                    "col": 0,
                ...
                for b in buttons:
                    if isinstance(b, dict) and b.get('row', 0) > cardlink_row and b.get('id') not in ('btn_renew_back', 'btn_back_main'):
                        b['row'] = b['row'] + 1
                buttons.append({
                    "id": "btn_renew_pay_yoomoney",
                    "label": "💳 ЮMoney",
            ...
            logger.info("Кнопка btn_renew_pay_yoomoney добавлена в дефолтную раскладку renew_payment")

    logger.info("Миграция v37 применена: добавлен платёжный метод YooMoney (ЮMoney)")


def migration_38(conn):
    """
    Миграция v38: добавление колонки yoomoney_payment_id в таблицу payments.

    Колонка хранит ID платежа в системе YooMoney для проверки статуса.
    """
    _add_column(conn, "payments", "yoomoney_payment_id TEXT")
    logger.info("Миграция v38 применена: добавлена колонка yoomoney_payment_id")


def migration_39(conn: sqlite3.Connection) -> None:
    """
    Добавляет колонку loyalty_paid_count в таблицу users для системы скидок.

    Хранит количество успешных оплат пользователя для расчёта скидки лояльности.
    """
    _add_column(conn, "users", "loyalty_paid_count INTEGER DEFAULT 0")
    logger.info("Миграция v39 применена: добавлена колонка loyalty_paid_count")


def migration_40(conn: sqlite3.Connection) -> None:
    """
    Миграция v40: Добавление кнопки «🔍 Узнать ID» в главное меню.
    """
    import json
    cursor = conn.execute("SELECT buttons_default, buttons_custom FROM pages WHERE page_key = 'main'")
    row = cursor.fetchone()
    if row:
        buttons_def_str, buttons_cust_str = row
        new_btn = {
            "id": "btn_show_id",
            "label": "🔍 Узнать ID",
            "color": "primary",
            ...
            "action_value": "cmd_show_id"
        }

        # Обновляем buttons_default
        if buttons_def_str:
            try:
                buttons_def = json.loads(buttons_def_str)
                if not any(b.get("id") == "btn_show_id" for b in buttons_def):
                    buttons_def.append(new_btn)
                    conn.execute(
                        "UPDATE pages SET buttons_default = ? WHERE page_key = 'main'",
                        (json.dumps(buttons_def, ensure_ascii=False),)
                    )
            except Exception as e:
                logger.error(f"Error parsing buttons_default for migration_40: {e}")

        # Обновляем buttons_custom
        if buttons_cust_str:
            try:
                buttons_cust = json.loads(buttons_cust_str)
                if not any(b.get("id") == "btn_show_id" for b in buttons_cust):
                    buttons_cust.append(new_btn)
                    conn.execute(
                        "UPDATE pages SET buttons_custom = ? WHERE page_key = 'main'",
                        (json.dumps(buttons_cust, ensure_ascii=False),)
                    )
            except Exception as e:
                logger.error(f"Error parsing buttons_custom for migration_40: {e}")
                
    logger.info("Миграция v40 применена: добавлена кнопка «🔍 Узнать ID» в главное меню")


def migration_41(conn: sqlite3.Connection) -> None:
    """
    Миграция v41: Изменение названия кнопки «🔍 Узнать ID» на «🔍 Смотреть ID».
    """
    import json
    cursor = conn.execute("SELECT buttons_default, buttons_custom FROM pages WHERE page_key = 'main'")
    row = cursor.fetchone()
    if row:
        ...
        if buttons_def_str:
            try:
                ...
                for btn in buttons_def:
                    if btn.get("id") == "btn_show_id":
                        btn["label"] = "🔍 Смотреть ID"
                        updated = True
                if updated:
                    conn.execute(
                        "UPDATE pages SET buttons_default = ? WHERE page_key = 'main'",
                ...
                logger.error(f"Error updating buttons_default for migration_41: {e}")
        ...
        if buttons_cust_str:
            try:
                ...
                logger.error(f"Error updating buttons_custom for migration_41: {e}")
                
    logger.info("Миграция v41 применена: изменено название кнопки на «🔍 Смотреть ID» в главном меню")
