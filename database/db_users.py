...
__all__ = [
    ...
    'get_loyalty_paid_count',
    'increment_loyalty_paid_count',
    'get_loyalty_discount_pct',
]
...
def get_user_balance(user_id: int) -> int:
    ...
    with get_db() as conn:
        ...
        row = cursor.fetchone()
        val = row['personal_balance'] if row else 0
...
def get_user_referral_coefficient(user_id: int) -> float:
    ...
    with get_db() as conn:
        ...
        row = cursor.fetchone()
        val = row['referral_coefficient'] if row else 1.0
...
# Пороги скидок: (минимальное кол-во оплат, процент скидки)
_LOYALTY_TIERS = [
    (12, 15),
    (6, 10),
    (3, 5),
...
def get_loyalty_paid_count(user_id: int) -> int:
    """
    Получает количество успешных оплат пользователя.

    Args:
        user_id: Внутренний ID пользователя

    Returns:
        Количество оплат (0 если колонка отсутствует или пользователь не найден)
    """
    with get_db() as conn:
        try:
            cursor = conn.execute(
                "SELECT loyalty_paid_count FROM users WHERE id = ?",
                (user_id,)
            )
            row = cursor.fetchone()
            return row['loyalty_paid_count'] if row else 0
        except Exception:
            return 0


def increment_loyalty_paid_count(user_id: int) -> None:
    """
    Увеличивает счётчик успешных оплат пользователя на 1.
    ...
    with get_db() as conn:
        try:
            conn.execute(
                "UPDATE users SET loyalty_paid_count = loyalty_paid_count + 1 WHERE id = ?",
                (user_id,)
            )
            logger.info(f"Loyalty count incremented for user {user_id}")
        except Exception as e:
            logger.warning(f"Failed to increment loyalty count for user {user_id}: {e}")


def get_loyalty_discount_pct(user_id: int) -> int:
    """
    Рассчитывает процент скидки на основе количества оплат.
    ...
    """
    paid_count = get_loyalty_paid_count(user_id)
    for min_count, pct in _LOYALTY_TIERS:
        if paid_count >= min_count:
            return pct
    return 0
