...
async def check_yoomoney_payment(callback: CallbackQuery, state: FSMContext):
    """Обрабатывает подтверждение оплаты YooMoney QuickPay по кнопке 'Я оплатил'.

    Схема QuickPay не предполагает автоматической серверной проверки
    (OAuth API не используется). Пользователь подтверждает оплату вручную,
    после чего заказ обрабатывается через общий process_payment_order.
    """
    from database.requests import (
    ...
    try:
        ...
        await safe_edit_or_send(callback.message, '⏳ Проверяем платёж...')

        # QuickPay: нет серверной проверки (OAuth не используется).
        # Обрабатываем платёж по подтверждению пользователя.
