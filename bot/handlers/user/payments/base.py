...
async def _send_payment_receipt(message: Message, order: dict, user_id: int):
    """Отправляет чек об оплате пользователю отдельным сообщением."""
    ...
    full_order = find_order_by_order_id(order_id) or order

    tariff_name = full_order.get('tariff_name') or full_order.get('name') or '—'
    duration = full_order.get('duration_days') or full_order.get('period_days') or '—'
    payment_type = full_order.get('payment_type', '')
    paid_at = full_order.get('paid_at', '')

    payment_label = _PAYMENT_TYPE_LABELS.get(payment_type, payment_type)

    # Сумма
    amount_rub = full_order.get('amount_rub')
    amount_cents = full_order.get('amount_cents')
    if amount_rub and float(amount_rub) > 0:
        amount_str = f"{int(float(amount_rub))} ₽"
    elif amount_cents and int(amount_cents) > 0:
        amount_str = f"{int(amount_cents) / 100:.2f} ₽"
    else:
        amount_str = full_order.get('amount_stars', '—')
    ...
    receipt_text = (
        f"🧾 <b>Чек об оплате</b>\n\n"
        f"📋 <b>Заказ:</b> <code>{escape_html(str(order_id))}</code>\n"
        f"💳 <b>Тариф:</b> {escape_html(str(tariff_name))}\n"
        f"⏳ <b>Срок:</b> {duration} дн.\n"
        f"💰 <b>Сумма:</b> {amount_str}\n"
        f"🏦 <b>Способ:</b> {escape_html(payment_label)}\n"
        f"📅 <b>Дата:</b> {date_str}\n\n"
    ...
    try:
        await message.answer(receipt_text, reply_markup=builder.as_markup(), parse_mode="HTML")
        logger.info(f"Receipt sent for order {order_id} to user {user_id}")
    except Exception as e:
        logger.warning(f"Failed to send receipt for order {order_id}: {e}")
...
async def finalize_payment_ui(message: Message, state: FSMContext, text: str, order: dict, user_id: int):
    ...
    if protocol in ('wireguard', 'amnezia') and key_id:
        ...
        await _send_payment_receipt(message, order, user_id)
        return
