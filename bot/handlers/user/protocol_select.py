...
async def protocol_vless_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_or_send(callback.message, '💳 <b>Купить ключ (VLESS)</b>\n\nВыберите тариф:', reply_markup=tariff_select_kb(rub_tariffs, back_callback='buy_key', is_payment_method_select=True, user_id=user_internal_id))
    await callback.answer()
...
async def select_payment_method_handler(callback: CallbackQuery, state: FSMContext):
    """Показывает все включённые способы оплаты для выбранного тарифа."""
    from database.requests import get_tariff_by_id, get_user_internal_id
    from bot.keyboards.user import payment_method_select_kb
    ...
    protocol = state_data.get('protocol', 'vless')
    ...
    await safe_edit_or_send(
        ...
        reply_markup=payment_method_select_kb(
            tariff_id=tariff_id,
            tariff_name=tariff['name'],
            price_rub=float(tariff.get('price_rub', 0)),
            protocol=protocol,
            back_callback=f'protocol_{protocol}',
