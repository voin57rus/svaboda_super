...
async def protocol_vless_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_or_send(callback.message, '💳 <b>Купить ключ (VLESS)</b><br><br>Выберите тариф:', reply_markup=tariff_select_kb(rub_tariffs, back_callback='buy_key', is_yoomoney=True, user_id=user_internal_id))
    await callback.answer()
...
async def _show_wg_tariffs(callback: CallbackQuery, state: FSMContext, amnezia: bool):
    ...
    if not rub_tariffs:
        ...
        return
    user_internal_id = get_user_internal_id(callback.from_user.id)
    await safe_edit_or_send(callback.message, f'💳 <b>Купить ключ ({proto_label})</b>\n\nВыберите тариф:', reply_markup=tariff_select_kb(rub_tariffs, back_callback='buy_key', is_yoomoney=True, user_id=user_internal_id))
    await callback.answer()
