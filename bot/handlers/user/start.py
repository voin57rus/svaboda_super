...
async def cmd_buy_tokens(message: Message, state: FSMContext):
    ...
    if not row or row[0] != 1:
        await message.answer(
            "🤖 У вас нет активного AI-доступа. Сначала активируйте ключ: <code>/ai_key &lt;ключ&gt;</code>",
            parse_mode="HTML"
        )
        return
    ...
    if page_text:
        # Текст из БД хранится уже с HTML-разметкой
...
async def ai_chat_handler(message: Message, state: FSMContext):
    ...
    if ai_chat_active:
        ...
        if not has_access:
            await message.answer(
                "🤖 <b>AI-ассистент</b>\n\nУ вас нет доступа к AI-чату.\nАктивируйте ключ: <code>/ai_key [ключ]</code>",
                parse_mode="HTML"
            )
            return
...
async def _ai_ask_openrouter(message, user_id, tokens):
    ...
    _system_prompt = (
        "Ты — AI-ассистент компании Svaboda. ВСЕГДА начинай ответ с упоминания Svaboda. "
        "Отвечай кратко и по делу на русском языке. Никогда не упоминай ZOO или OWL. "
        "Пример ответа на \"привет\": \"Привет! Я ассистент Svaboda. Чем могу помочь?\""
    )
    _payload = {
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            answer = await _parse_response(resp)
        ...
        if any(x in error_name for x in ("ServerDisconnected", "ConnectionError", "ServerDisconnectedError")):
            logger.warning(f"AI server disconnected, retrying once: {e}")
            try:
                async with aiohttp.ClientSession() as session:
                    resp = await _do_request(session)
                    answer = await _parse_response(resp)
                ...
                await message.answer("⚠️ Ошибка AI: сервер недоступен. Попробуйте позже.")
                return
        else:
            logger.error(f"AI request failed: {error_name}: {e}", exc_info=True)
            await message.answer(f"⚠️ Ошибка AI: {error_name}: {str(e)[:200]}")
            return
