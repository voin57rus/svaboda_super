...
from database.requests import (
    get_or_create_user, is_user_banned, get_setting,
...
async def _render_main_page(target, force_new: bool = False):
    ...
    show_trial = is_trial_enabled() and get_trial_tariff_id() and not has_used_trial(user_id)
...
async def _show_ai_tariff(callback, state, tariff, price, tokens):
    ...
    if is_admin:
        ...
        text = (
            ...
            f"📦 Тариф: <b>{TARIFF_NAMES_RU.get(current_tariff, 'нет').upper()}</b>\n"
            f"💰 Токены: <b>{current_tokens:,}</b>\n\n"
            "Напишите сообщение — AI ответит без ограничений."
        )
    ...
    if has_access:
        ...
        if current_tariff and current_tariff.upper() != TARIFF_NAMES_RU.get(tariff, '').upper():
            ...
            text = f"⛔ У вас активен тариф <b>{display_tariff}</b>.\n\n" + text
...
async def cmd_ai_key(message: Message, state: FSMContext):
    ...
    if len(args) < 2:
        await message.reply("🔑 Использование: /ai_key <ключ>")
        return
    ...
    c.execute("SELECT ai_access FROM users WHERE telegram_id=?", (user_id,))
    if c.fetchone() and c.fetchone()[0] == 1:
        conn.close()
        await message.answer("ℹ️ У вас уже есть доступ.")
        return

    c.execute("SELECT id, tokens, tariff FROM ai_keys WHERE key=? AND is_active=1", (key,))
    row = c.fetchone()

    if not row:
        conn.close()
        await message.answer("❌ Ключ не найден или активирован.")
        return

    key_id, tokens, tariff = row

    c.execute("UPDATE ai_keys SET is_active=0, activated_by=? WHERE id=?", (user_id, key_id))
    c.execute(
        "UPDATE users SET ai_access=1, ai_tokens=?, ai_key=?, ai_tariff=? WHERE telegram_id=?",
        (tokens, key, tariff, user_id)
    )
...
async def cmd_buy_tokens(message: Message, state: FSMContext):
    ...
    if not row or row[0] != 1:
        await message.answer("❌ Сначала активируйте AI: /ai_key <ключ>")
        return
...
async def ai_chat_handler(message: Message, state: FSMContext):
    """Обрабатывает обычные сообщения как AI чат."""
    import sqlite3
    import aiohttp
    from config import ADMIN_IDS
    ...
    # если админ в FSM админки — не перехватываем
    if is_admin:
    ...
    if is_admin:
        if tokens <= 0:
            await message.answer("⚠️ Токены закончились /buy_tokens", parse_mode="HTML")
            return
    ...
    # нет токенов
    if tokens <= 0:
        await message.answer("⚠️ Нет токенов /buy_tokens", parse_mode="HTML")
        return

    # предупреждение
    if tokens <= 10:
        await message.answer(
            f"⚠️ Осталось {tokens} токенов",
            parse_mode="HTML"
        )
...
async def _ai_ask_openrouter(message, user_id, tokens):
    """Запрос к OpenRouter + списание токена."""
    import sqlite3
    import aiohttp
    import os
    ...
    api_key = get_ai_api_key() or os.getenv("OPENROUTER_API_KEY")

    if not api_key:
        await message.answer("⚠️ Нет API ключа OpenRouter")
        return
    ...
    try:
        async with aiohttp.ClientSession() as session:
            resp = await session.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "Ты AI ассистент Svaboda. Отвечай кратко на русском."
                        },
                        {
                            "role": "user",
            ...
            if resp.status != 200:
                ...
                await message.answer(f"⚠️ AI ошибка: {msg}")
                return

            choices = data.get("choices")
            if not choices:
                await message.answer("⚠️ Пустой ответ AI")
                return

            answer = choices[0]["message"]["content"]

    except asyncio.TimeoutError:
        await message.answer("⚠️ AI не ответил вовремя")
        return

    except Exception as e:
        await message.answer(f"⚠️ Ошибка AI: {type(e).__name__}")
        return
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE users SET ai_tokens = MAX(ai_tokens - 1, 0) WHERE telegram_id=?",
        (user_id,)
    )
    conn.commit()
    conn.close()
