...
def _format_user_card(user: dict) -> tuple[str, any]:
    ...
    try:
        balance_rub = int(balance_cents) / 100 if balance_cents else 0
    ...
    if total_payments > 0:
        total_usd = int(payment_stats.get('total_amount_cents', 0) or 0) / 100
        total_stars = int(payment_stats.get('total_amount_stars', 0) or 0)
        total_rub = int(payment_stats.get('total_amount_rub', 0) or 0)
...
async def start_balance_add(callback: CallbackQuery, state: FSMContext):
    ...
    balance_rub = int(current_balance) / 100
...
async def process_balance_amount(message: Message, state: FSMContext):
    ...
    if operation == 'deduct':
        ...
        new_balance_rub = int(new_balance) / 100
