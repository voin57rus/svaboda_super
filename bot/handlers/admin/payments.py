...
async def show_payments_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_or_send(callback.message,
        text,
        reply_markup=payments_menu_kb(stars, crypto, cards, qr, monthly_reset, demo, wata, platega, cardlink, yoomoney)
    )
...
async def show_yoomoney_management_menu(callback: CallbackQuery, state: FSMContext):
    """Показывает меню управления оплатой через YooMoney."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    if client_id:
        if len(client_id) >= 8:
            client_display = f"Установлен ✅ (<code>{escape_html(client_id[:4])}...{escape_html(client_id[-4:])}</code>)"
        else:
            client_display = "Установлен ✅"
    else:
        client_display = "❌ Не задан"

    if secret_key:
        if len(secret_key) >= 8:
            secret_display = f"Установлен ✅ (<code>{escape_html(secret_key[:4])}...{escape_html(secret_key[-4:])}</code>)"
        else:
            secret_display = "Установлен ✅"
    else:
        secret_display = "❌ Не задан"
    ...
    text = (
        "💰 <b>Оплата YooMoney</b>\n\n"
        "Приём платежей через YooMoney (ЮKassa) — удобный способ оплаты "
        "картами, СБП и другими способами.\n\n"
        "📋 <b>Как получить доступ:</b>\n"
        "1. Зарегистрируйтесь: <a href=\"https://yoomoney.ru/joinups/?source=sva\">yoomoney.ru</a>\n"
        "2. Создайте приложение в разделе «API-интеграция»\n"
        "3. Получите Client ID и Secret Key\n"
        "4. Укажите их в кнопках ниже\n\n"
        f"{status_emoji} Статус: <b>{status_text}</b>\n"
        f"🆔 Client ID: {client_display}\n"
        f"🔐 Secret Key: {secret_display}\n\n"
        "Выберите действие:"
    )
...
async def yoomoney_mgmt_toggle(callback: CallbackQuery, state: FSMContext):
    """Включает/выключает оплату через YooMoney."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    if not current:
        ...
        if not client_id or not client_id.strip() or not secret_key or not secret_key.strip():
            await callback.answer("❌ Сначала укажите Client ID и Secret Key!", show_alert=True)
            return
    ...
    status = "включена ✅" if new_value == '1' else "выключена"
    await callback.answer(f"YooMoney-оплата {status}")
    await show_yoomoney_management_menu(callback, state)


@router.callback_query(F.data == "admin_yoomoney_mgmt_edit_client_id")
async def yoomoney_edit_client_id(callback: CallbackQuery, state: FSMContext):
    """Запрашивает Client ID YooMoney."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return

    await state.set_state(AdminStates.yoomoney_setup_client_id)
    await state.update_data(last_menu_msg_id=callback.message.message_id)

    await safe_edit_or_send(
        callback.message,
        "🆔 <b>Введите Client ID YooMoney</b>\n\n"
        "Найдите в личном кабинете YooMoney: <b>Настройки → API-интеграция</b>\n\n"
        "<i>Значение будет частично скрыто после сохранения.</i>",
...
@router.message(AdminStates.yoomoney_setup_client_id)
async def yoomoney_setup_client_id_handler(message: Message, state: FSMContext):
    """Обрабатывает ввод Client ID YooMoney."""
    from bot.utils.text import get_message_text_for_storage

    client_id = get_message_text_for_storage(message, 'plain').strip()

    if len(client_id) < 4:
        await safe_edit_or_send(message, "❌ Слишком короткий Client ID. Попробуйте ещё раз.")
        return
    ...
    set_setting('yoomoney_client_id', client_id)
...
@router.callback_query(F.data == "admin_yoomoney_mgmt_edit_secret_key")
async def yoomoney_edit_secret_key(callback: CallbackQuery, state: FSMContext):
    """Запрашивает Secret Key YooMoney."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return

    await state.set_state(AdminStates.yoomoney_setup_secret_key)
    await state.update_data(last_menu_msg_id=callback.message.message_id)

    await safe_edit_or_send(
        callback.message,
        "🔐 <b>Введите Secret Key YooMoney</b>\n\n"
        "Найдите в личном кабинете YooMoney: <b>Настройки → API-интеграция</b>\n\n"
        "<i>Значение будет частично скрыто после сохранения.</i>",
        reply_markup=back_and_home_kb("admin_payments_yoomoney"),
    )
...
@router.message(AdminStates.yoomoney_setup_secret_key)
async def yoomoney_setup_secret_key_handler(message: Message, state: FSMContext):
    """Обрабатывает ввод Secret Key YooMoney."""
    from bot.utils.text import get_message_text_for_storage

    secret_key = get_message_text_for_storage(message, 'plain').strip()

    if len(secret_key) < 8:
        await safe_edit_or_send(message, "❌ Слишком короткий Secret Key. Попробуйте ещё раз.")
        return
    ...
    set_setting('yoomoney_secret_key', secret_key)
