...
async def render_tariff_view(message: Message, tariff_id: int, state: FSMContext):
    ...
    await safe_edit_or_send(message, 
        "\n".join(lines),
        reply_markup=tariff_view_kb(tariff_id, bool(tariff['is_active']), groups_count > 1, protocol=tariff.get('protocol', 'vless'))
    )
...
async def clear_tariffs_by_protocol(callback: CallbackQuery, state: FSMContext):
    ...
    # Показываем результат и обновлённый список
    await safe_edit_or_send(
        callback.message,
        f"{proto_emoji.get(protocol, '')} <b>Тарифы {proto_label.get(protocol, protocol)} очищены!</b>\n\n"
        f"🗑 Удалено: {count} тарифов\n\n"
        f"Можно создавать новые.",
        reply_markup=tariffs_list_kb(get_all_tariffs(include_hidden=True))
    )
