...
def user_view_kb(telegram_id: int, vpn_keys: List[Dict[str, Any]], is_banned: bool, balance_cents: int=0, referral_coefficient: float=1.0) -> InlineKeyboardMarkup:
    ...
    balance_rub = int(balance_cents) / 100
    ...
    builder.row(InlineKeyboardButton(text='🗑 Полностью удалить пользователя', callback_data=f'admin_user_delete_full:{telegram_id}'))
