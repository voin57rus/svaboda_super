...
def buy_key_kb(
    ...
    # YooMoney — переход к выбору тарифа
    if yoomoney_enabled:
        builder.row(
...
def tariff_select_kb(tariffs: list, back_callback: str = "buy_key", order_id: str = None, is_cards: bool = False, is_crypto: bool = False, is_balance: bool = False, is_qr: bool = False, groups_data: list = None, is_demo: bool = False, is_wata: bool = False, is_platega: bool = False, is_cardlink: bool = False, is_wireguard: bool = False, is_yoomoney: bool = False, user_id: int = None, is_payment_method_select: bool = False) -> InlineKeyboardMarkup:
    """
    ...
    Args:
        ...
        user_id: Внутренний ID пользователя для расчёта скидки лояльности
    """
    # Рассчитываем скидку лояльности один раз для всех тарифов
    _loyalty_discount_pct = 0
    if user_id:
        from database.db_users import get_loyalty_discount_pct
        _loyalty_discount_pct = get_loyalty_discount_pct(user_id)

    builder = InlineKeyboardBuilder()
    
    def _fmt_rub_price(price_rub):
        """Форматирует цену в рублях с учётом скидки лояльности."""
        if _loyalty_discount_pct > 0:
            discounted = price_rub * (1 - _loyalty_discount_pct / 100)
            return f"{int(discounted)} ₽ (-{_loyalty_discount_pct}%)"
        return f"{price_rub} ₽"
        ...
        for tariff in tariff_list:
            if is_crypto:
                ...
                if price_rub is None or price_rub <= 1:
                    continue
                price_display = _fmt_rub_price(price_rub)
                prefix = "cards_pay"
                emoji = '💳'
            elif is_demo:
                price_rub = tariff.get('price_rub')
                ...
                prefix = "yoomoney_pay"
                emoji = '💳'
            elif is_payment_method_select:
                price_rub = tariff.get('price_rub')
                if price_rub is None or price_rub < 1:
                    continue
                price_display = _fmt_rub_price(price_rub)
                prefix = "select_payment_method"
                emoji = '💰'
            elif is_wireguard:
                # WireGuard / AmneziaWG — оплата через Telegram Stars
                # price_cents это USD центах, конвертируем в Stars (1 USD ≈ 100 Stars)
                price_usd = tariff['price_cents'] / 100
                stars_amount = max(1, int(price_usd * 100))
                price_display = f"{stars_amount} ⭐"
                prefix = "wg_pay"
                emoji = "🔑"
            elif is_balance:
                price_rub = tariff.get('price_rub')
...
def payment_method_select_kb(tariff_id: int, tariff_name: str, price_rub: float, protocol: str = "vless", back_callback: str = "buy_key", order_id: str = None, user_id: int = None) -> InlineKeyboardMarkup:
    """
    Клавиатура выбора способа оплаты для конкретного тарифа.
    Показывает все включённые способы оплаты.
    ...
    Args:
        ...
        back_callback: Callback для кнопки «Назад»
        order_id: ID существующего ордера
        user_id: Внутренний ID пользователя для расчёта скидки
    """
    from database.db_settings import (
        ...
        is_cardlink_configured, is_yoomoney_enabled, is_demo_payment_enabled
    )
    ...
    if _loyalty_discount_pct > 0:
        discounted = price_rub * (1 - _loyalty_discount_pct / 100)
        price_display = f"{int(discounted)} ₽ (-{_loyalty_discount_pct}%)"
    else:
        price_display = f"{int(price_rub)} ₽"

    builder = InlineKeyboardBuilder()

    # Заголовок — название и цена тарифа
    builder.row(
        InlineKeyboardButton(text=f"📦 {tariff_name} — {price_display}", callback_data="noop")
    )
    ...
    if is_crypto_configured():
        builder.row(InlineKeyboardButton(
            text="🪙 Оплатить USDT",
            callback_data=f"crypto_pay:{tariff_id}:{order_id}" if order_id else f"crypto_pay:{tariff_id}"
        ))
    ...
    if is_stars_enabled():
        builder.row(InlineKeyboardButton(
            ...
            callback_data=f"stars_pay:{tariff_id}:{order_id}" if order_id else f"stars_pay:{tariff_id}"
        ))
    ...
    if is_cards_enabled():
        builder.row(InlineKeyboardButton(
            text="💳 Карта (Telegram)",
            callback_data=f"cards_pay:{tariff_id}:{order_id}" if order_id else f"cards_pay:{tariff_id}"
        ))

    # ЮКасса QR / СБП
    if is_yookassa_qr_configured():
        builder.row(InlineKeyboardButton(
            text="📱 ЮКасса (QR/СБП)",
            callback_data=f"qr_pay:{tariff_id}:{order_id}" if order_id else f"qr_pay:{tariff_id}"
        ))

    # WATA
    if is_wata_configured():
        builder.row(InlineKeyboardButton(
            text="🌊 WATA (Карта/СБП)",
            callback_data=f"wata_pay:{tariff_id}:{order_id}" if order_id else f"wata_pay:{tariff_id}"
        ))

    # Platega (СБП)
    if is_platega_configured():
        builder.row(InlineKeyboardButton(
            text="💸 Platega (СБП)",
            callback_data=f"platega_pay:{tariff_id}:{order_id}" if order_id else f"platega_pay:{tariff_id}"
        ))

    # Cardlink (Карта/СБП)
    if is_cardlink_configured():
        builder.row(InlineKeyboardButton(
            text="🔗 Cardlink (Карта/СБП)",
            callback_data=f"cardlink_pay:{tariff_id}:{order_id}" if order_id else f"cardlink_pay:{tariff_id}"
        ))

    # YooMoney
    if is_yoomoney_enabled():
        builder.row(InlineKeyboardButton(
            text="💳 ЮMoney",
            callback_data=f"yoomoney_pay:{tariff_id}:{order_id}" if order_id else f"yoomoney_pay:{tariff_id}"
        ))

    # Демо оплата
    if is_demo_payment_enabled():
        builder.row(InlineKeyboardButton(
            text="🏦 Демо оплата",
            callback_data=f"demo_pay:{tariff_id}:{order_id}" if order_id else f"demo_pay:{tariff_id}"
        ))
...
def renew_tariff_select_kb(tariffs: list, key_id: int, order_id: str = None, is_cards: bool = False, is_crypto: bool = False, is_balance: bool = False, is_qr: bool = False, is_demo: bool = False, is_wata: bool = False, is_platega: bool = False, is_cardlink: bool = False, is_yoomoney: bool = False, user_id: int = None) -> InlineKeyboardMarkup:
    """
...
def qr_qr_kb(order_id: str, back_callback: str = "buy_key", qr_url: str = None) -> InlineKeyboardMarkup:
    """
    Клавиатура страницы QR-оплаты ЮКасса.
...
# ============================================================================
# YOOMONEY-ОПЛАТА (ЮMoney через ссылку)
# ============================================================================

def yoomoney_qr_kb(order_id: str, back_callback: str = "buy_key", qr_url: str = None) -> InlineKeyboardMarkup:
    """
    Клавиатура страницы оплаты YooMoney (ЮMoney).

    Args:
        order_id: Наш внутренний order_id
        back_callback: Каллбэк для кнопки «Назад»
        qr_url: Ссылка для оплаты (URL)
    """
    builder = InlineKeyboardBuilder()
    ...
    builder.row(
        InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_yoomoney:{order_id}")
    )
    builder.row(
        InlineKeyboardButton(text="⬅️ Назад", callback_data=back_callback),
        InlineKeyboardButton(text="📋 На главную", callback_data="start")
...
def protocol_select_kb() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="🟢 WireGuard", callback_data="protocol_wireguard"),
    )
    builder.row(
...
def yoomoney_pay_kb(order_id: str, back_callback: str = "buy_key", confirmation_url: str = None) -> InlineKeyboardMarkup:
    """
    Клавиатура страницы оплаты YooMoney.

    Args:
        order_id: Наш внутренний order_id
        back_callback: Каллбэк для кнопки «Назад»
        confirmation_url: Ссылка для перехода к оплате
    """
    builder = InlineKeyboardBuilder()

    if confirmation_url:
        builder.row(
            InlineKeyboardButton(text="💳 Оплатить YooMoney", url=confirmation_url)
        )
