...
from database.requests import (
    ...
    get_cardlink_credentials, get_yoomoney_credentials,
    ...
    get_loyalty_discount_pct, increment_loyalty_paid_count
)
...
async def process_payment_order(order_id: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    # Увеличиваем счётчик лояльности
    increment_loyalty_paid_count(user_internal_id)
...
def calculate_loyalty_discount(user_id: int, price_rub: float) -> tuple[float, int]:
    """
    Рассчитывает скидку лояльности для пользователя.

    Args:
        user_id: Внутренний ID пользователя
        price_rub: Оригинальная цена в рублях

    Returns:
        Кортеж (discounted_price, discount_pct):
        - discounted_price: цена со скидкой (или оригинальная, если скидки нет)
        - discount_pct: процент скидки (0 если скидки нет)
    """
    pct = get_loyalty_discount_pct(user_id)
    if pct <= 0:
        return price_rub, 0
    discounted = price_rub * (1 - pct / 100)
    return round(discounted, 2), pct
...
# ============================================================================
# YOOMONEY — оплата ЮMoney через REST API (https://yoomoney.ru)
# ============================================================================
...
async def create_yoomoney_payment(
    ...
    """
    Создаёт QuickPay ссылку для оплаты через YooMoney.

    Формат: https://yoomoney.ru/transfer/quickpay?...

    Args:
        amount_rub: Сумма в рублях
        order_id: Наш внутренний order_id (используется как label)
        description: Описание платежа
        bot_name: Username бота (для построения return_url)

    Returns:
        Словарь с ключами:
            - yoomoney_label: label платежа (= order_id)
            - qr_url: QuickPay ссылка для оплаты
            - status: 'pending'

    Raises:
        ValueError: Если учётные данные не настроены
    """
    shop_id, secret_key = get_yoomoney_credentials()
    if not shop_id or not secret_key:
        raise ValueError("YooMoney: не настроены YOOMONEY_SHOP_ID или YOOMONEY_SECRET_KEY")
    ...
    params = {
        ...
        "sum": f"{float(amount_rub):.2f}",
        "label": label,
        "successURL": return_url,
    }

    query = urllib.parse.urlencode(params)
    quickpay_url = f"https://yoomoney.ru/transfer/quickpay?{query}"

    logger.info(
        f"YooMoney QuickPay создан: order_id={order_id}, amount={amount_rub} RUB, label={label}"
    )
...
def verify_yoomoney_notification(params: dict, secret_key: str) -> bool:
    """
    Verifies YooMoney notification signature using HMAC-SHA1.

    YooMoney Notification Service sends a POST to the configured URL
    after each payment. The sha1_hash is computed as:
    SHA1(notification_type&operation_id&amount&currency&datetime&sender&codepro&label&secret_key)

    Args:
        params: dict with notification fields from the POST body
        secret_key: the secret_key configured in YooMoney account
    ...
    string_to_hash = '&'.join(parts) + '&' + secret_key

    computed_hash = hashlib.sha1(string_to_hash.encode('utf-8')).hexdigest()
...
async def check_yoomoney_payment_status(order_id: str) -> str:
    """
    Проверяет статус платежа YooMoney.

    Использует проверку подписи уведомления (notification) вместо OAuth.
    Если уведомление было получено — платёж подтверждён.
    ...
    Returns:
        Нормализованный статус: 'pending' | 'succeeded' | 'canceled'
    """
    from database.db_payments import find_order_by_yoomoney_label

    order = find_order_by_yoomoney_label(order_id)
    if not order:
        return 'pending'
    ...
    yoomoney_payment_id = order.get('yoomoney_payment_id')
    if yoomoney_payment_id:
        return 'succeeded'
