...
async def check_yoomoney_payment_status(order_id: str) -> str:
    """
    Проверяет статус платежа YooMoney по label через operation-history API.
    ...
    # secret_key используется как OAuth-токен для operation-history API
    headers = {
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=payload, headers=headers) as response:
            try:
                ...
                text = await response.text()
                logger.error(f"YooMoney operation-history: невозможно разобрать ответ ({response.status}): {text}")
                raise RuntimeError("YooMoney API вернул некорректный ответ")

            if response.status != 200:
                error_desc = data.get('error') or data.get('message') or str(data)
                logger.error(f"YooMoney operation-history ошибка {response.status}: {error_desc}")
