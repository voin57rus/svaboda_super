...
ACTION_REGISTRY: Dict[str, str] = {
    ...
    "cmd_help":           "help",
    "cmd_ai_standard":    "ai_standard",
    "cmd_ai_premium":     "ai_premium",
    "cmd_ai_vip":         "ai_vip",
    "cmd_show_id":        "show_id",
}
...
# - order_id, telegram_id, is_admin, и другие параметры
# =============================================================================


def _hide_if_admin(ctx: dict) -> Optional[dict]:
    """Если админ — скрыть кнопку (админ не должен видеть оплату)."""
    if ctx.get('is_admin'):
        return None
    return {"callback_data": "_placeholder_"}
...
def _resolve_pay_crypto(ctx: dict) -> Optional[dict]:
    ...
    # Админ не должен видеть кнопки оплаты
    if ctx.get('is_admin'):
        return None
...
def _resolve_renew_pay_yoomoney(ctx: dict) -> Optional[dict]:
    """Кнопка продления через YooMoney."""
    from database.requests import is_yoomoney_configured
    ...
    return {"callback_data": f"renew_yoomoney_tariff:{key_id}"}
