...
def generate_vless_json_full(config: Dict[str, Any]) -> str:
    """Генерирует полный JSON-конфигурацию для VLESS (как в 3X-UI панели)."""
    stream = config.get('stream_settings', {})
    network = stream.get('network', 'tcp')
    security = stream.get('security', 'none')
    flow = config.get('flow', '')
    remark = _get_remark(config)
...
def generate_wg_config_text(
    ...
    """
    Генерирует текст конфигурации обычного WireGuard (без AmneziaWG).

    Args:
        client_private_key: Приватный ключ клиента
        client_ip: IP клиента (напр. 10.8.1.3)
    ...
    lines.append(f"Address = {client_ip}/32")
...
def generate_wg_link(
    ...
    """
    Генерирует строку подключения для WireGuard.
    Формат: wg://base64
    """
    import base64
    config = generate_wg_config_text(
        client_private_key, client_ip, server_public_key,
        preshared_key, endpoint, dns
    )
...
# ============================================================================
# AMNEZIAWG — генерация конфигов с обфускацией
# ============================================================================

def generate_amnezia_wg_config_text(
    client_private_key: str,
    client_ip: str,
    server_public_key: str,
    preshared_key: str,
    endpoint: str = "87.120.165.232:47981",
    dns: str = "8.8.4.4, 9.9.9.9",
    mtu: int = 1420,
    jc: int = 5,
    jmin: int = 10,
    jmax: int = 50,
    ...
    """
    Генерирует текст конфигурации AmneziaWG.
    Включает параметры обфускации (Jc, Jmin, Jmax, S1, S2, H1-H4).
    Keenetic игнорирует неизвестные поля, Amnezia-клиент их использует.
    """
    lines = ["[Interface]"]
    lines.append(f"PrivateKey = {client_private_key}")
    lines.append(f"Address = {client_ip}/32")
    ...
    lines.append(f"Jc = {jc}")
    lines.append(f"Jmin = {jmin}")
    lines.append(f"Jmax = {jmax}")
