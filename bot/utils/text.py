...
async def safe_edit_or_send(
    ...
    Args:
        ...
        photo: Фото (file_id, URL, путь к файлу или InputFile). Если передано — отправляем медиа-сообщение
    """
    from aiogram.types import FSInputFile
    
    # Если photo — это локальный путь к файлу, оборачиваем в FSInputFile
    if photo and isinstance(photo, str) and not photo.startswith('http') and not photo.startswith('AgAC'):
        import os
        # Пробуем как абсолютный путь, потом относительно корня проекта
        if not os.path.isfile(photo):
            abs_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), photo)
