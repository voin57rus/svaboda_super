...
async def render_page(
    ...
    # 0. Защита админа от попадания на страницы оплаты
    if page_key in ('prepayment', 'renew_payment', 'renew_payment_unavailable'):
        ctx = context or {}
        if ctx.get('is_admin'):
            logger.info(f"Admin redirected from '{page_key}' to protocol selection")
            from bot.handlers.user.protocol_select import buy_key_handler
            # Создаём фейковый callback если target — это Message
            if isinstance(target, CallbackQuery):
                await buy_key_handler(target, None)
            return
