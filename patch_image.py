...
# 1. Add "Стартовая картинка" button to admin_edit_texts menu
...
# 2. Add edit_image handler before edit_text_start
handler_code = '''
...
async def edit_image_start(callback: CallbackQuery, state: FSMContext):
    ...
    current_image = row[0] if row and row[0] else (row[1] if row else None)
